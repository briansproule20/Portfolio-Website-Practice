    ...
    <section className="max-w-5xl mx-auto px-4 py-12">
      ...
      {/* Repositories */}
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {data.repos.map((repo) => (
            <div key={repo.name} className="rounded-xl shadow-lg p-6 bg-[var(--card)] border-2 border-[var(--accent)] hover:border-[var(--highlight)] transition-colors group">
              <div className="flex items-start justify-between mb-3">
                <h3 className="text-lg font-semibold text-[var(--foreground)] group-hover:text-[var(--highlight)] transition-colors">
                  <Link href={repo.url} target="_blank" rel="noopener noreferrer" className="hover:underline">
                    {repo.name}
                  </Link>
                </h3>
                {repo.stars > 0 && (
                  <span className="flex items-center text-sm text-[var(--accent)]">
              ...
              {repo.description && (
                <p className="text-[var(--accent)] text-sm mb-3 line-clamp-2">
                  {repo.description}
                </p>
              )}
              ...
              <div className="flex items-center justify-between text-sm">
                {repo.language && (
                  <div className="flex items-center">
                    ...
                    />
                    <span className="text-[var(--accent)]">{repo.language}</span>
                  </div>
                )}
                <span className="text-[var(--accent)] opacity-75">
